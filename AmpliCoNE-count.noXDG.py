#!/usr/bin/python
#NOTE: E. Kopania 18 Sep 2020 - I changed all instances of numpy.mean() to numpy.nanmean() to ignore NAs when calculating mean instead of returning NA; this requires adding "dtype=float" to the nanmean() function
#NOTE: E. Kopania 17 Jan 2021 - Did the same for numpy.sum() to numpy.nansum() (line 376)
#NOTE: E. Kopania 12 Feb 2021 - Removed any lines dividing by XDG copy number because this sometimes results in dividing by zero for females; commented out lines 391 and 392, edited line 403 to print "NA" instead of XDG copy number
import sys
import re
import pandas
import numpy
...
parser.add_argument('--CHR', '-c', required=True, help=' Chromosome annotation as found in the BAM header file. ', choices=['Y','chrY','X','chrX','5','chr5','14','chr14'])
...
print(Control_data.shape) #Get dimensions of control data to know how many Y chr positions are control regions

Summary_data=Summary_data.values
Control_data=Control_data.values
#mean_Control=numpy.mean(Control_data[:,3]) 
mean_Control=numpy.nanmean(Control_data[:,3], dtype=float)
print("Control data mean copy number:")
print(mean_Control)
mean_Summary=numpy.nanmean(Summary_data[:,3], dtype=float)
print("All data mean copy number:")
print(mean_Summary)
...
for i in range(1,100):
	...
	if len(counts_temp) > 0 :
		...
		gcm=numpy.nanmean(counts_temp, dtype=float)
	else:
		gcm=0
	print i, gcm
...
###FOR CHECKING GC CORRECTION
#GC_out="GCcor_Summary.txt"
#with open(GC_out, "w") as GCfile:
#        for i in GCcor_Summary_data:
#                # #print i[2]+"\t"+str(i[1])
#                GCfile.write(i)
#		GCfile.write("\n")
#GCcor_out="GC_correction_output.txt"
#with open(GCcor_out,"w") as GCcorFile:
#	for i in Correction:
#		print(str(i))
#		GCcorFile.write(str(i))
#		GCcorFile.write("\n")
...
def Control_region_coverage(Y_Summary_data) : # Ychr_summary=['Position', 'Mappability', 'GC', 'RSC','Informative']
		...
		return numpy.nanmean(Control_region[:,3], dtype=float)
...
def Get_Informative_coverage(Gene_info,Y_Summary_data) : 		# Y_Summary_data=['Position', 'Mappability', 'GC', 'RSC','Informative']; Gene_info=[START,	END,	TYPE]
	...
	return [Gene_info[2],numpy.nanmean(Informative_data[:,3], dtype=float),str(Gene_info[2])+"_"+str(Gene_info[0])+"_"+str(Gene_info[1])]                          #return(gene_info,mean coverage)
...
def Get_ControlGene_coverage(Gene_info,Y_Summary_data) : 		# Y_Summary_data=['Position', 'Mappability', 'GC', 'RSC','Informative']; Gene_info=[START,	END,	TYPE]
	...
	return [Gene_info[2],numpy.nanmean(Unique_data[:,3], dtype=float),str(Gene_info[2])+"_"+str(Gene_info[0])+"_"+str(Gene_info[1])]
...
print(XDG_Genes)
XDG_Control_coverage=numpy.nanmean(XDG_Genes[:,1], dtype=float)
...
with open(AG_out, "w") as AGfile:  
	...
	for family in sorted(Family_list.keys()):
		...
		AGfile.write(family+"\t"+str(numpy.nansum(Family_set[:,1]))+"\t"+"NA"+"\n") #str(numpy.nansum(Family_setXDG[:,1]))+ "\n")
...
#ALL_out="AllGenes_CopyNumber.txt"
